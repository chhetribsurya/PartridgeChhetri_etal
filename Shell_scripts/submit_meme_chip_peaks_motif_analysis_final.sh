...
for i in "${!LIB_REP1[@]}"; do
	...
	bsub $BSUB_OPTIONS -o "$IDR_DIR/motif_corrected_idr.${R1}_${R2}.truerep.out" -J "Motif analysis from IDR of $R1 $R2 " $RUN_PATH/motif_pipeline/motif_analysis_final_pipeline.sh  $IDR_DIR/${R1}.filt.nodup.srt.SE_VS_${R2}.filt.nodup.srt.SE.IDR0.02.filt.narrowPeak.gz $R1 $R2
