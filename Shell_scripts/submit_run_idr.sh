...
for i in "${!LIB_REP1[@]}"; do
	...
	# True Rep IDR
	bsub $BSUB_OPTIONS -o "$IDR_DIR/idr.${R1}_${R2}.truerep.out" -J "IDR $R1 $R2 true reps" ./Idr.main.sh $IDR_DIR/$R1.filt.nodup.srt.SE.narrowPeak.gz $IDR_DIR/$R2.filt.nodup.srt.SE.narrowPeak.gz $IDR_DIR/${R1}_${R2}.Rep0.narrowPeak.gz signal.value $IDR_DIR

	# Rep 1 self-pseudoreps (Rep1 psuedorep1 vs Rep1 pseudorep2)
	bsub $BSUB_OPTIONS -o "$IDR_DIR/idr.${R1}_${R2}.rep1_pseudo.out" -J "IDR $R1 $R2 rep1 pseudo" ./Idr.main.sh $IDR_DIR/$R1.filt.nodup.SE.pr1.narrowPeak.gz $IDR_DIR/$R1.filt.nodup.SE.pr2.narrowPeak.gz $IDR_DIR/$R1.filt.nodup.srt.SE.narrowPeak.gz signal.value $IDR_DIR

	# Rep 2 self-pseudoreps (Rep2 pseudorep1 vs Rep2 pseudorep2) 
	bsub $BSUB_OPTIONS -o "$IDR_DIR/idr.${R1}_${R2}.rep2_pseudo.out" -J "IDR $R1 $R2 rep2 pseudo" ./Idr.main.sh $IDR_DIR/$R2.filt.nodup.SE.pr1.narrowPeak.gz $IDR_DIR/$R2.filt.nodup.SE.pr2.narrowPeak.gz $IDR_DIR/$R2.filt.nodup.srt.SE.narrowPeak.gz signal.value $IDR_DIR

	# Pooled psuedoreps ( Pooled pseudorep1 (PR1 of Rep1 + PR1 of Rep2) vs Pooled true tag align file(Rep1_TA_file + Rep2_TA_file)
	bsub $BSUB_OPTIONS -o "$IDR_DIR/idr.${R1}_${R2}.pool_pseudo.out" -J "IDR $R1 $R2 pooled pseudo" ./Idr.main.sh $IDR_DIR/${R1}_${R2}.Rep0.pr1.narrowPeak.gz $IDR_DIR/${R1}_${R2}.Rep0.pr2.narrowPeak.gz $IDR_DIR/${R1}_${R2}.Rep0.narrowPeak.gz signal.value $IDR_DIR
