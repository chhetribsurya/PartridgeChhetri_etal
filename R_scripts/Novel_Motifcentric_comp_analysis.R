...
cisbp_tomfile <- "Novel_motifs_cisbp_tomtom_fraction_table_redo.txt"

tomtom_df = fread(file.path(output_dir, cisbp_tomfile), sep="\t")
names(tomtom_df) <- c("motif_fraction", "tom_Evalue")

output_file_name <- file.path(output_dir, "Distant_motif_similarity_plot_from_cisBP_redo.pdf")       
...
cisbp_scan_tomfile <- "Novel_motifs_combined_tomtom_cisbp_scan_df_top50_redo.txt"
cisbp_scan_tomfile <- "Novel_motifs_combined_tomtom_cisbp_scan_df_top5000_redo.txt"
...
output_file_name <- file.path(output_dir, "Distant_motif_similarity_plot_with_293_motifs_top50tomtomHitsCompare_redo.pdf")       
output_file_name <- file.path(output_dir, "Distant_motif_similarity_plot_with_293_motifs_top5000tomtomHitsCompare_redo.pdf")       
...
# Adding one more dimension of CMO_E_value instead of DNA motifs:
tomtom_df$log10_CMO_E_value = log10(as.numeric(as.character(tomtom_df$cmo_E_value)))

pdf(output_file_name)
#tomtom_df$anno <- factor(tomtom_df$anno, levels=c("mismatch","match","no_match"))
ggplot(tomtom_df, aes(y=as.factor(), x=tom_E_value_final, label = tf_motif_id )) +
        geom_point(aes(color=anno), size=0.9) + geom_text(check_overlap = TRUE, size=1.5) +
        scale_color_manual(values=c("green","orange","red")) +
        #geom_rug(color="darkgray", sides="b") +
        geom_vline(xintercept=x_cutoff,linetype=2) +
...
concordant_df <- tomtom_df_subset %>% filter(anno_new=="concordance")
discordant_df <- tomtom_df_subset %>% filter(anno_new=="discordance")
nomatch_df <- tomtom_df[tomtom_df$anno == "no_match",]
...
pdf(output_file_name)
hist(discordant_df$adj_offset_dist, col=rgb(1,0,0,0.25), xlim=c(0,50), ylim=c(0,25), breaks=seq(min(vec2),max(vec2)+2, bin_width), main="Offset Distribution", ylab="Number of Motifs", xlab="Median offset distance(bp)")
...
output_file_name <- file.path(output_dir, "Median_offset_dist_plot_all_combined_redo.pdf")       
pdf(output_file_name)
...
hist(concordant_df$adj_offset_dist, col=rgb(0,0,1,0.25), xlim=c(0,60), ylim=c(0,25), breaks=seq(min(vec1),max(vec1)+2, bin_width), main="Offset Distribution", ylab="Number of Motifs", xlab="Median offset distance(bp)")
hist(discordant_df$adj_offset_dist, xlim=c(0,60), ylim=c(0,25), col=rgb(1,0,0,0.25),breaks=seq(min(vec2),max(vec2)+2, bin_width), add=T)
hist(nomatch_df$adj_offset_dist, xlim=c(0,60), ylim=c(0,25), col="green", breaks=seq(min(vec2),max(vec2)+2, bin_width), add=T)
...
# Centrimo value based (off peak distance):
...
output_dir <- "/Users/suryachhetri/Dropbox/for_genemodels/motifs_compinfo_analysis/novel_motif_analysis"
motif_factor_df <- fread(file.path(output_dir, "Motif_factor_and_regulatory_region_association_redo.txt"))
output_file_name <- file.path(output_dir, "Motif_factor_and_regulatory_region_association_redo.pdf")       
pdf(output_file_name)
...
# Redo Motif and Factors relation with Prom/Enh/Prom-Enh
...
motif_factor_df <- fread(file.path(output_dir, "concordant_discordant_chromatinstate_anno.txt"))
#names(motif_factor_df) <- c("TF_name", "Annotation", "anno_new")
output_file_name <- file.path(output_dir, "Motif_factor_chromatinstate_association_redo.pdf")       
pdf(output_file_name)

ggplot(motif_factor_df, aes(x=manual_anno, fill=Annotation)) +
  geom_bar(width=0.5) + scale_y_continuous(breaks=seq(0,200,20)) +
  labs(x="", y="Count", fill="State Annotation") #+
  #scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9")
