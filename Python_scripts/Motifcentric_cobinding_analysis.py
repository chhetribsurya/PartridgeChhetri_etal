...
main_dir = "/gpfs/gpfs1/home/schhetri/for_chris/batch_I/motifs_compinfo_analysis/cobinding_analysis"
...
def cobinding_tf_peaks_count_perc_overlap_info(TF_file_list_of_interest, combined_TF_bedfile):
	...
	for each_file in tf_file_list:
		...
		tf_tf_intersect = tf_bedfile.intersect(combined_tf_bedfile, wa=True, wb=True)
		print tf_tf_intersect.head()

		pybed_df = pd.read_csv(tf_tf_intersect.fn, sep="\t", header=None)
		header = df_read.columns.tolist() + ["peak_chrom", "peak_start", "peak_end", "cobind_motif_id", "cobind_tf_name", "cobind_strand", "cobind_tfmotif_id"]
