...
tomtom_fraction_df.to_csv(join(output_dir, "Novel_motifs_cisbp_tomtom_fraction_table.txt"), sep="\t", header=True, index=False)
final_dbf_motifinfo["tom_E-value"].median() # 50 percentile
# final_dbf_motifinfo["tom_E-value"].mean()


# In [239]: final_crcf_motifinfo.shape
# Out[239]: (43, 13)

# In [240]: final_dbf_motifinfo.shape
# Out[240]: (293, 13)

# combined_significant_motifinfo_df = combined_motifinfo_df.loc[(combined_motifinfo_df["E-value"].astype(float) < 1e-05) & (combined_motifinfo_df["cmo_E-value"].astype(float) < 1e-10)]
# (-combined_motifinfo_df["cmo_E-value"].astype(float)).describe(percentiles=[0.25,0.5,0.75])
# combined_significant_motifinfo_df["E-value"].astype(float).describe(percentiles=[0.05,0.95])
# combined_motifinfo_df.loc[~combined_motifinfo_df["TF_NAME"].isin(anno_df["Target"])]

######################################################
# Not Required for novel motif analysis purpose:
######################################################
...
alltf_fimo_df = pd.read_pickle(join(output_dir,"final_tfcombined_fimocoords_df.pkl"))
...
def final_tomtom_motifs_model_for_top5scan(tomtom_file, tf_name, num_top_hit_scan, **kwargs):
    motif_list = []
    for key, values in kwargs.iteritems():
        motif_list.append(values)
    df = pd.read_csv(tomtom_file, sep="\t")
    ...
    select_cols = ["tf_name", "motif_id", "tophit", "E-value"]
...
for key, val in tomtom_filedict.iteritems():
    ...
    TF_name = key
    tomtom_file = val
    tomtom_df = final_tomtom_motifs_model_for_top5scan(tomtom_file, TF_name, 500, motif1=1, motif2=2, motif3=3, motif4=4, motif5=5)
    name = "tom_"
    tomtom_df = tomtom_df.add_prefix(name)
    tomtom_df.rename(columns={name+"motif_id": "motif_id"}, inplace=True) # for common-val based merge
...
combined_tomtom_cisbp_scan_df.to_csv(join(output_dir, "Novel_motifs_combined_tomtom_cisbp_scan_df_top500.txt"), sep="\t", header=True, index=False)

combined_tomtom_cisbp_scan_df.to_pickle(join(output_dir,"combined_tomtom_cisbp_scan_df_top500.pkl"))
combined_tomtom_cisbp_scan_df = pd.read_pickle(join(output_dir, "combined_tomtom_cisbp_scan_df_top500.pkl"))


#################################################

# For Motif Similarity/Dissimilarity Heatmap Analysis:
...
def final_tomtom_motifs_model_for_top5scan(tomtom_file, tf_name, num_top_hit_scan, **kwargs):
    ...
    for key, val in grp_object:
        grp_df = grp_object.get_group(key)
        grp_df_sorted = grp_df.sort_values(["E-value"])
        df_list.append(grp_df_sorted)
    combined_grp_df = pd.concat(df_list, ignore_index=True)
...
final_dbf_cisbp_df["TF_NAME_2"] = final_dbf_cisbp_df["TF_NAME"].str.replace("_human", "").str.replace("\[FLAG\]", "").str.upper().str.replace(r"_.*", "")
final_dbf_cisbp_df["tf_name_2"] = final_dbf_cisbp_df["tf_name"].str.upper().str.replace(r"_.*", "").str.replace("\(", "").str.replace("\)", "").str.replace(r"\..*", "")
#final_dbf_cisbp_df["tf_name_2"] = final_dbf_cisbp_df["tf_name"].fillna("N/A", inplace=True)

final_dbf_cisbp_df["MOTIF_ID_2"] = final_dbf_cisbp_df["MOTIF_ID"].str.replace("MOTIF", "")
final_dbf_cisbp_df["tf_motif_id"] = final_dbf_cisbp_df["TF_NAME_2"] + "." + final_dbf_cisbp_df["MOTIF_ID_2"]
final_dbf_cisbp_df["tom_E-value_y"].fillna(1, inplace=True)
final_dbf_cisbp_df.to_csv(join(output_dir, "Novel_motifs_combined_tomtom_cisbp_comprehensive_df.txt"), sep="\t", header=True, index=False)

#Pickle files:
final_dbf_cisbp_df.to_pickle(join(output_dir,"Novel_motifs_combined_tomtom_cisbp_comprehensive_df.pkl"))
final_dbf_cisbp_df = pd.read_pickle(join(output_dir, "Novel_motifs_combined_tomtom_cisbp_comprehensive_df.pkl"))

# Heatmap reshaping:
final_dbf_cisbp_df_heatmap = final_dbf_cisbp_df.pivot(index="tf_motif_id", columns="tf_name_2", values="tom_E-value_y")
final_dbf_cisbp_df_heatmap.rename(columns={np.nan:"NA_Hit"}, inplace=True)
#final_dbf_cisbp_df_heatmap2 = final_dbf_cisbp_df_heatmap.fillna(1.5, inplace=True)

final_dbf_cisbp_df_heatmap.to_csv(join(output_dir, "Novel_motifs_combined_tomtom_cisbp_allscan_df_heatmap.txt"), sep="\t", header=True, index=True)
